    ...
    return {
        ...
        groupData: function(){
            $("#location_id_filter, #bulan, #tahun, #searching").on('change', function() {
                ...
                $.ajax({
                    url : App.baseUrl+"fuel_plan/dataList",
                    type : "POST",
                    data : {
                    ...
                    success : function(data) {
                        ...
                        var url = App.baseUrl + 'fuel_plan/exportExcel';
                        var url_location = "";
                        ...
                        if(data.status){
                            ...
                            for (let i = 0; i < data.data.length; i++) {
                                ...
                                for(let x = 1; x <= data.akhir_bulan; x++){
                                    nilai_body += '<td class="text-center">'+data.data[i].datas[x]+'</td>';
                                }
                                nilai_body += '</tr>';
                            }
                            var total_head = '<tr>';
                            total_head += '<th class="text-center">Total Fuel</th>';
                            total_head += '<th class="text-center">' +data.data_total["total"]+'</th>';
                            for(let y = 1; y <= data.akhir_bulan; y++){
                                total_head += '<th class="text-center">'+data.data_total[y]+'</th>';
                            }
                            total_head += '</tr>';
                        }
        ...
        loadData: function(){
            $(".unit, .tahun_input, .bulan_input").on('change', function() {
                ...
                if(unit != "" && tahun != "" && bulan != ""){
                    App.id_unit = unit;
                    $.ajax({
                        url : App.baseUrl+"fuel_plan/get_data",
                        type : "POST",
                        data : {
                            "tahun" : tahun,
                            "bulan" : bulan,
                ...
                $("#form-create").validate({
                    ...
                    submitHandler : function(form) {
                        ...
                        App.confirm("Apakah Anda Yakin Menginput Data Ini?",function(){
                            ...
                            $.ajax({
                                url : App.baseUrl + 'fuel_plan/input_data',
                                type : "POST",
                                dataType : "json",
                                data : {
                                    "location_id" : location_id, 
        ...
        onChangeLocation: function(){
            $(".location_id").on("change", function(){
                var id = $(this).val();
                $.ajax({
                    url: App.baseUrl+'fuel_plan/getUnit',
                    type: 'POST',
                    data: {id: id},
                })
