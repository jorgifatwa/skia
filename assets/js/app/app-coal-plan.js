	...
	return {
		...
		groupData: function(){
			$("#location_id_filter, #bulan, #tahun, #searching").on('change', function() {
				...
				if (location_id != "") {
					$.ajax({
						url : App.baseUrl+"coal_plan/dataList",
						type : "POST",
						data : {
						...
						success : function(data) {
							...
							var url = App.baseUrl + 'coal_plan/exportExcel';
							var url_location = "";
							...
							if(data.status){
								...
								var total_head = '<tr>';
								total_head += '<th class="text-center">Total Coal Plan</th>';
		...
		loadData: function(){
			$(".unit, .tahun_input, .bulan_input").on('change', function() {
				...
				if(unit != "" && tahun != "" && bulan != ""){
					App.id_unit = unit;
					$.ajax({
						url : App.baseUrl+"coal_plan/get_data",
						type : "POST",
						data : {
							"tahun" : tahun,
							"bulan" : bulan,
				...
				$("#form-create").validate({
					...
					submitHandler : function(form) {
						...
						App.confirm("Apakah Anda Yakin Menginput Data Ini?",function(){
							...
							$.ajax({
								url : App.baseUrl + 'coal_plan/input_data',
								type : "POST",
								dataType : "json",
								data : {
									"location_id" : location_id, 
								...
								success : function(data) {
									if(data.status){
										toastr.success("Data Coal Plan Telah Ditambahkan", 'Berhasil', { timeOut: 3000 })
									}else{
										toastr.error("Data Coal Plan Gagal Ditambahkan", 'Gagal', { timeOut: 3000 })
									}
									$(".unit").trigger('change');
									$("#nilai_input").val("");
								},
		...
		onChangeLocation: function(){
			$(".location_id").on("change", function(){
				var id = $(this).val();
				$.ajax({
					url: App.baseUrl+'coal_plan/getUnit',
					type: 'POST',
					data: {id: id},
				})
